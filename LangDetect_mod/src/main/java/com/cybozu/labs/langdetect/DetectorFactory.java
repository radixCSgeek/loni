...
public class DetectorFactory {
    ...
    public static void loadProfileFromJarResources(String profileListResourcePath) throws LangDetectException {
        ...
        try {
            ...
            throw new LangDetectException(ErrorCode.NeedLoadProfileError, "Could not read profile resource list: " + profileListResourcePath + " : IOException : "+e.getLocalizedMessage());
        }
        if (listFiles.isEmpty())
            throw new LangDetectException(ErrorCode.NeedLoadProfileError, "Not found profile from resource list: " + profileListResourcePath);
            
        int langsize = listFiles.size(), index = 0;
        for (String file: listFiles) {
            InputStream is = null;
            try {
                is = DetectorFactory.class.getResourceAsStream(file);
                LangProfile profile = JSON.decode(is, LangProfile.class);
                addProfile(profile, index, langsize);
                ++index;
            } catch (JSONException e) {
